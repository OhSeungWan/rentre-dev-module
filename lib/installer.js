...
/**
 * BMAD core 모듈 존재 여부 확인
 */
async function validateBmadCore(bmadDir) {
  // core/tasks/workflow.xml 또는 core 디렉토리 존재 확인
  const coreDir = path.join(bmadDir, 'core');
  const workflowXml = path.join(coreDir, 'tasks', 'workflow.xml');
  const manifestPath = path.join(bmadDir, '_cfg', 'manifest.yaml');

  if (await fs.pathExists(workflowXml)) {
    return { valid: true, reason: 'workflow.xml found' };
  }

  if (await fs.pathExists(coreDir)) {
    return { valid: true, reason: 'core directory found' };
  }

  if (await fs.pathExists(manifestPath)) {
    return { valid: true, reason: 'manifest.yaml found' };
  }

  return { valid: false, reason: 'BMAD core module not found' };
}
...
async function install(options = {}) {
  ...
  let bmadDirName = DEFAULT_BMAD_DIR;
  if (!useDefaults) {
    const { bmadPath } = await inquirer.prompt([
      {
        type: 'input',
        name: 'bmadPath',
        message: 'BMAD 설치 경로를 입력하세요:',
        default: DEFAULT_BMAD_DIR,
      },
    ]);
    bmadDirName = bmadPath.trim() || DEFAULT_BMAD_DIR;
  }

  const bmadDir = path.join(projectRoot, bmadDirName);

  // 2. BMAD core 모듈 존재 확인
  const spinner = ora('BMAD core 모듈 확인 중...').start();

  if (!(await fs.pathExists(bmadDir))) {
    spinner.fail(`BMAD 디렉토리가 존재하지 않습니다: ${chalk.bold(bmadDir)}`);
    console.log(chalk.red('\n❌ 설치 실패: BMAD core 모듈이 먼저 설치되어 있어야 합니다.'));
    console.log(chalk.dim('   BMAD를 먼저 설치한 후 다시 시도해주세요.\n'));
    process.exit(1);
  }

  const validation = await validateBmadCore(bmadDir);
  if (!validation.valid) {
    spinner.fail(`BMAD core 모듈을 찾을 수 없습니다: ${chalk.bold(bmadDir)}`);
    console.log(chalk.red('\n❌ 설치 실패: 해당 경로에 BMAD core 모듈이 없습니다.'));
    console.log(chalk.dim('   올바른 BMAD 설치 경로인지 확인해주세요.\n'));
    process.exit(1);
  }

  spinner.succeed(`BMAD core 확인 완료: ${chalk.bold(bmadDir)}`);

  // 3. 설치 경로 설정
  const targetDir = path.join(bmadDir, MODULE_NAME);
  const bmadFolderName = `${bmadDirName}/${MODULE_NAME}`;
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      await copyWithPlaceholders(srcPath, destPath, bmadFolderName, '{project-root}');
    } else {
      const ext = path.extname(entry.name).toLowerCase();
      ...
      if (textExtensions.includes(ext)) {
        let content = await fs.readFile(srcPath, 'utf8');
        content = replacePlaceholders(content, bmadFolderName, '{project-root}');
        await fs.writeFile(destPath, content, 'utf8');
      } else {
        await fs.copy(srcPath, destPath);
      }
  ...
  const compiledAgents = await compileAndCopyAgents(moduleSource, targetDir, bmadFolderName);
  compileSpinner.succeed(`에이전트 컴파일 완료 (${compiledAgents.length}개)`);

  // 8. 워크플로우/Task 정보 수집
  const collectSpinner = ora('워크플로우 정보 수집 중...').start();
  const workflows = await collectWorkflows(targetDir, bmadFolderName);
  const tasks = await collectTasks(targetDir, bmadFolderName);
  collectSpinner.succeed(`워크플로우 ${workflows.length}개, Task ${tasks.length}개 발견`);

  // 9. Claude Code 명령 생성
  const claudeSpinner = ora('Claude Code 명령 생성 중...').start();
  const commandCounts = await generateClaudeCommands(process.cwd(), bmadFolderName, compiledAgents, workflows, tasks);
  ...
  const moduleConfig = {
    ...
    version: '1.0.0',
    ...userConfig,
    data_path: `{project-root}/${bmadFolderName}/data`,
    tasks_path: `{project-root}/${bmadFolderName}/data/tasks`,
    backlogs_path: `{project-root}/${bmadFolderName}/data/backlogs`,
    reports_path: `{project-root}/${bmadFolderName}/data/reports`,
    guides_path: `{project-root}/${bmadFolderName}/data/guides`,
  };
...
async function findInstalledModule(projectRoot) {
  // 가능한 BMAD 디렉토리들 확인
  const possibleDirs = ['.bmad', 'bmad', '.bmad-method'];

  for (const dir of possibleDirs) {
    const targetDir = path.join(projectRoot, dir, MODULE_NAME);
    if (await fs.pathExists(targetDir)) {
      return { targetDir, bmadDir: dir };
    }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        if (await fs.pathExists(targetDir)) {
          // config.yaml 존재 여부로 실제 rentre-dev 모듈인지 확인
          const configPath = path.join(targetDir, 'config.yaml');
          if (await fs.pathExists(configPath)) {
            return { targetDir, bmadDir: entry.name };
          }
        }
      }
    }
...
async function status() {
  ...
  const { targetDir, bmadDir } = installed;
  console.log(chalk.green('✓ rentre-dev:'), targetDir);
  console.log(chalk.dim(`  BMAD 경로: ${bmadDir}`));
