...
const INSTALL_BASE_DIR = 'bmad-custom-modules-src';
...
async function install(options = {}) {
  ...
  // 커스텀 모듈 경로 입력받기
  let installBaseDirName = INSTALL_BASE_DIR;
  if (!useDefaults) {
    const { customPath } = await inquirer.prompt([
      {
        type: 'input',
        name: 'customPath',
        message: 'BMAD 커스텀 모듈 경로를 입력하세요:',
        default: INSTALL_BASE_DIR,
      },
    ]);
    installBaseDirName = customPath.trim() || INSTALL_BASE_DIR;
  }

  const spinner = ora('설치 환경 확인 중...').start();
  const installBaseDir = path.join(projectRoot, installBaseDirName);
  const targetDir = path.join(installBaseDir, MODULE_NAME);
  const bmadFolderName = `${installBaseDirName}/${MODULE_NAME}`;
  spinner.succeed(`설치 경로: ${chalk.bold(targetDir)}`)
...
async function uninstall() {
  ...
  // bmad-custom-modules-src/rentre-dev 경로에서 찾기
  const projectRoot = process.cwd();
  const targetDir = path.join(projectRoot, INSTALL_BASE_DIR, MODULE_NAME);
