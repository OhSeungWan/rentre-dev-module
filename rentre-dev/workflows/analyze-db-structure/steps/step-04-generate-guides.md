---
name: 'step-04-generate-guides'
description: '가이드 문서 생성'

# Path Definitions
workflow_path: '{module_path}/workflows/analyze-db-structure'

# File References
thisStepFile: '{workflow_path}/steps/step-04-generate-guides.md'
nextStepFile: '{workflow_path}/steps/step-05-complete.md'
workflowFile: '{workflow_path}/workflow.md'

# Task References
advancedElicitationTask: '{project-root}/{bmad_folder}/core/tasks/advanced-elicitation.xml'
partyModeWorkflow: '{project-root}/{bmad_folder}/core/workflows/party-mode/workflow.md'

# Template References
output_template: '{workflow_path}/templates/guide-summary.md'

# Output References
guides_folder: '{module_path}/data/guides'
---

# Step 4: 가이드 문서 생성

**Progress: Step 4 of 5** - Next: 완료 및 검증

## STEP GOAL:

분석 결과를 바탕으로 에이전트용 가이드 문서들을 생성합니다.

## MANDATORY EXECUTION RULES (READ FIRST):

### Universal Rules:

- 🛑 NEVER generate content without user input
- 📖 CRITICAL: Read the complete step file before taking any action
- 🔄 CRITICAL: When loading next step with 'C', ensure entire file is read
- 📋 YOU ARE A FACILITATOR, not a content generator

### Role Reinforcement:

- ✅ You are a 데이터베이스 분석 전문가
- ✅ We engage in collaborative dialogue, not command-response
- ✅ You bring 문서화 전문성, user brings 팀 요구사항

### Step-Specific Rules:

- 🎯 Focus on 문서 생성 - 각 문서 완료 후 진행
- 🚫 FORBIDDEN to skip any guide document
- 💬 Show each document and ask for confirmation
- 📁 Ensure guides_folder exists before writing

## EXECUTION PROTOCOLS:

- 🎯 출력 디렉토리 먼저 확인/생성
- 💾 각 문서 생성 후 저장
- 📖 사용자에게 문서 내용 확인 받기
- 🚫 FORBIDDEN 문서 생성 실패 무시 금지

## CONTEXT BOUNDARIES:

- Available context: 스키마 분석 + 계층 매핑 결과
- Focus: 가이드 문서 생성
- Limits: 검증은 다음 스텝에서 수행
- Dependencies: step-02, step-03 완료

---

## Sequence of Instructions (Do not deviate, skip, or optimize)

### 1. 출력 디렉토리 확인

```yaml
action:
  - Check if {guides_folder} exists
  - If not exists: Create directory
  - Confirm write permissions
```

**If directory creation fails:**

> "**⚠️ 가이드 폴더 생성 실패**
>
> 경로: {guides_folder}
>
> 수동으로 폴더를 생성하거나 권한을 확인해 주세요."

**→ [R] Retry 옵션 제시**

### 2. Summary Guide 생성

**파일:** `{guides_folder}/backlog-guide-summary.md`

#### A. 문서 내용 구성

```markdown
# Backlog Guide Summary

> Generated by Rentre-Dev on {date}
> Database: {database_name}

## Overview

| 항목              | 값                 |
| ----------------- | ------------------ |
| 데이터베이스 이름 | {database_name}    |
| 데이터베이스 ID   | {database_id}      |
| 백로그 유형 수    | {types_count}      |
| 전체 속성 수      | {properties_count} |
| 관계 속성 수      | {relations_count}  |

## Backlog Types

{types_table}

## Hierarchy
```

{hierarchy_diagram}

```

## Properties Reference

### Core Properties
{core_properties_table}

### Relation Properties
{relation_properties_table}

## Agent Navigation Guide

{navigation_quick_reference}
```

#### B. 문서 저장 및 표시

문서 내용을 저장하고 사용자에게 표시:

> "**📄 Summary Guide 생성됨**
>
> 파일: `backlog-guide-summary.md`
>
> {document_preview}
>
> 내용이 적절한가요?"

**Wait for user confirmation.**

### 3. Hierarchy Map 생성

**파일:** `{guides_folder}/hierarchy-map.md`

#### A. 문서 내용 구성

```markdown
# Backlog Hierarchy Map

> Generated by Rentre-Dev on {date}

## Visual Hierarchy
```

{hierarchy_diagram_detailed}

````

## Relation Property Mappings

| 관계 속성 | 연결 방향 | 설명 |
| --------- | --------- | ---- |
{relation_mappings_table}

## Navigation Flowchart

### Finding Parent Items
1. Get current item's "{parent_relation}" property
2. Query database with ID from relation

### Finding Child Items
1. Query database
2. Filter: "{child_relation}" contains current item ID

## Cross-Reference Table

{cross_reference_table}

## Agent Usage Examples

### Example: Find all Tasks under a Story
```yaml
action:
  tool: mcp__notionApi__API-post-database-query
  params:
    database_id: {database_id}
    filter:
      property: "{parent_relation}"
      relation:
        contains: "{story_id}"
````

````

#### B. 문서 저장 및 표시

> "**📄 Hierarchy Map 생성됨**
>
> 파일: `hierarchy-map.md`
>
> {document_preview}"

### 4. Type-Specific Guides 생성

식별된 각 백로그 유형별 가이드 생성:

**파일 패턴:** `{guides_folder}/guide-{type_lowercase}.md`

#### A. 각 유형별 문서 구성

```markdown
# {Type} Guide

> Generated by Rentre-Dev on {date}

## Overview

- **유형:** {type_name}
- **목적:** {type_purpose}
- **계층 위치:** {hierarchy_position}

## Required Fields

| 필드 | 타입 | 설명 |
| ---- | ---- | ---- |
{required_fields_table}

## Optional Fields

{optional_fields_table}

## Parent/Child Relationships

### Parent
- 유형: {parent_type}
- 관계 속성: {parent_relation}

### Children
- 유형: {child_types}
- 관계 속성: {child_relation}

## Status Workflow

{status_workflow_diagram}

## Notion MCP API Examples

### Find all {type_name}s
```yaml
tool: mcp__notionApi__API-post-database-query
params:
  database_id: {database_id}
  filter:
    property: "Type"
    select:
      equals: "{type_name}"
````

### Create new {type_name}

```yaml
tool: mcp__notionApi__API-post-page
params:
  parent:
    database_id: { database_id }
  properties:
    # ... required properties
```

## Best Practices

{type_best_practices}

```

#### B. 진행 상황 표시

각 유형별 가이드 생성 시:

> "**📄 {Type} Guide 생성됨** ({current}/{total})
>
> 파일: `guide-{type}.md`"

### 5. 생성 완료 보고

모든 문서 생성 후 요약:

> "**✅ 가이드 문서 생성 완료**
>
> ━━━━━━━━━━━━━━━━━━━━━━━
>
> **생성된 파일:**
>
> 1. `backlog-guide-summary.md` - 전체 요약
> 2. `hierarchy-map.md` - 계층 구조 맵
> {type_guides_list}
>
> **총 {total_files}개 파일 생성됨**
>
> 경로: `{guides_folder}/`
>
> ━━━━━━━━━━━━━━━━━━━━━━━"

---

### 6. Present MENU OPTIONS

Display: "**Select an Option:** [C] Continue - 완료 및 검증 [V] View - 파일 내용 확인 [R] Regenerate - 다시 생성 [X] Exit - 종료"

#### Menu Handling Logic:

- IF C: Load, read entire file, then execute {nextStepFile}
- IF V: Show list of files, let user select one to view
- IF R: Re-execute from section 1
- IF X: End workflow with summary
- IF Any other comments or queries: help user respond then redisplay menu

#### EXECUTION RULES:

- ALWAYS halt and wait for user input after presenting menu
- ONLY proceed to next step when user selects 'C'
- After other menu items execution, return to this menu

---

## CRITICAL STEP COMPLETION NOTE

ONLY WHEN [C continue option] is selected and [모든 가이드 문서가 생성됨], will you then load and read fully `{nextStepFile}` to execute and begin 완료 및 검증 단계.

---

## 🚨 SYSTEM SUCCESS/FAILURE METRICS

### ✅ SUCCESS:

- 출력 디렉토리 확인/생성 완료
- Summary Guide 생성 및 저장
- Hierarchy Map 생성 및 저장
- 모든 Type Guide 생성 및 저장
- 사용자 확인 완료

### ❌ SYSTEM FAILURE:

- 디렉토리 생성 실패 무시
- 문서 생성 누락
- 저장 실패 무시
- 사용자 확인 없이 진행
- **CRITICAL**: 스텝 파일 일부만 읽음

**Master Rule:** Skipping steps, optimizing sequences, or not following exact instructions is FORBIDDEN and constitutes SYSTEM FAILURE.
